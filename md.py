...
class system:
    ...
    def calculate_temperature(self, time):
        ...
        kinetic_energy = np.sum(velocities**2) # need to account for mass if mass not equal to 1
        T = kinetic_energy / (3 * len(self.particles))
        return T
    ...
    def verlet_simulate(self, dt, t, unit_cell_size, lattice_dim):
        ...
        with open('md.txt', 'w') as md_file, open('temp.txt', 'w') as temp_file:
            for i in range(n_iter):
                ...
                time = (i + 1) * dt
                T = self.calculate_temperature(time)
